...
public class F7 implements RuleI{
	...
	public ArrayList<EventI> match(EventBaseI eb) {
		...
		for (int i = 0 ; i < eb.numberOfEvents() && (dIntervention == null ) ; i++) {
			...
			if (e instanceof DemandeInterventionFeu && ((DemandeInterventionFeu) e).getCorrelatedEvents().get(0).hasProperty("type")&& ((DemandeInterventionFeu) e).getCorrelatedEvents().get(0).hasProperty("position")
					&& ((DemandeInterventionFeu) e).getCorrelatedEvents().get(0).getPropertyValue("type")== TypeOfFire.House
			) {
				dIntervention = e;
			}
		}
	...
	public void act(ArrayList<EventI> matchedEvents, CorrelatorStateI c) throws Exception {
		PompierCorrelatorStateI pompierCorrelatorState = (PompierCorrelatorStateI) c;
		DemandeInterventionFeu demandeI = (DemandeInterventionFeu) matchedEvents.get(0);
		ArrayList<EventI> correlateEvent = demandeI.getCorrelatedEvents();
		pompierCorrelatorState.declancheFirstAlarme((AbsolutePosition) correlateEvent.get(0).getPropertyValue("position"), TypeOfFirefightingResource.StandardTruck);
	}
